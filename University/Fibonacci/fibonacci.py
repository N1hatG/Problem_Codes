...
#fibonacci_calc funtions is for calculating the result of the fib(n)
...
def fibonacci_naive(n, outputf):
    ...
    if second >= 1:
        #for decomposing the equations fib(n)=fib(n-1) + fib(n-2) into parts
        fibonacci_naive(first, outputf)
        fibonacci_naive(second, outputf)

#naive function is the part where result is calculated and output is written
...
def fibonacci_eager(n, calcs, outputf):
    ...
    if second in calcs.keys():
        #when the fib(n-2) is known, fib(n) has to be known, too.
        calcs[n] = fibonacci_calc(n)

def eager(input_nums):
    with open(sys.argv[3], 'w') as outputf:
        #calcs dictionary is for containing the previous calculations
        calcs = {
            1: 1,
            2: 1
        }
...
def main():
    ...
    for i in range(len(input_nums)):
        input_nums[i] = int(input_nums[i][0]) #turned the list of strings into integers
    inputf.close()
